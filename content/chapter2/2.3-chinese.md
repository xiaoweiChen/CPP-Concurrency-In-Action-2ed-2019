...
这里对C++17给出一个建议，就是添加一个joining_thread的类型，这个类型与`std::thread`类似；与`std::thread`不同是的添加了析构函数，就类似于scoped_thread。对此委员会成员并没有达成共识，所以这个类没有添加入C++17标准中(C++20仍旧对这种方式进行探讨，不过名称为`std::jthread`)，不过这个类实现起来也不是很困难。下面我就来对这个类进行实现：
